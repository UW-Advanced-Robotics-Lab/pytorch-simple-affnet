...
def main():
    ...
    for i, (image, target) in enumerate(data_loader):
        ...
        depth_16bit = np.squeeze(np.array(target['depth_16bit'], dtype=np.uint16))
        depth = np.squeeze(np.array(target['depth'], dtype=np.uint8))
        ...
        # depth
        img_stats = depth_16bit
        img_stats = img_stats.reshape(1, -1)
        img_stats = np.array(img_stats, dtype=np.float64)
        img_stats = img_stats[img_stats!=0].reshape(1, -1)
        ...
        count_b += hist_b[0]
        # depth
        hist_d = np.histogram(depth_16bit, bins=nb_bins, range=[0, 255])
        count_d += hist_d[0]
